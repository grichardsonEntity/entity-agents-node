...
export const quinnConfig: AgentConfig = {
  ...
  allowedBashPatterns: [
    'git *', 'gh *', 'docker *', 'docker-compose *', 'kubectl *', 'helm *',
    'ssh *', 'ping *', 'traceroute *', 'curl *', 'nmap *', 'dig *',
    'wireguard *', 'tailscale *', 'ansible *', 'ansible-playbook *',
    'terraform *', 'haproxy *', 'nginx *', 'istioctl *', 'linkerd *',
    'prometheus *', 'grafana-cli *',
  ],
  githubLabels: [
    'infrastructure', 'networking', 'devops', 'deployment', 'kubernetes',
...
- DNS failover tested before production
- DR plans reviewed quarterly
